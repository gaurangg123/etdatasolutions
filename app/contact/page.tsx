...
import { Mail, Phone, Globe, MapPin, Clock, CheckCircle2, AlertTriangle, Loader2, MessageSquare, ChevronDown } from 'lucide-react'
...
// Item 25: consistent height 48px, rounded 8px, border focus brand color, subtle bg
const inputBase = [
...
// Item 27: contact items with colored icon circles
const contactItems = [
...
// Item 28: service tag colors matching site-wide tag style
const serviceTags = [
  { label:'Staffing & VA',    color:'bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-500/20' },
  { label:'RPO',              color:'bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-500/20' },
  { label:'Data Entry',       color:'bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-200 dark:border-sky-500/20' },
  { label:'Excel Automation', color:'bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-200 dark:border-sky-500/20' },
  { label:'Manual QA',        color:'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20' },
  { label:'Web Testing',      color:'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20' },
  { label:'Snowflake',        color:'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/20' },
  { label:'Databricks',       color:'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/20' },
  { label:'Power BI',         color:'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/20' },
  { label:'Tableau',          color:'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/20' },
]
    ...
    <>
      ...
      <section className="relative overflow-hidden py-20 md:py-28 bg-white dark:bg-ink-950">
        ...
        <Container className="relative">
          ...
          {/* Item 5: H1 proper scale */}
          <AnimateIn delay={0.07}>
            <h1 className="text-[2.8rem] md:text-[3.6rem] lg:text-[4.2rem] font-[800] tracking-[-0.045em] leading-[1.05] text-ink-900 dark:text-ink-50 mb-5 max-w-[620px] text-balance">
      ...
      <Section bg="white" id="form" label="Contact form">
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-10">
            ...
            {/* ── Left — contact details — item 27: icon circles + hover underline ── */}
            <div className="flex flex-col gap-4">
                  ...
                  <div className="divide-y divide-ink-100 dark:divide-ink-800">
                    {contactItems.map(item => (
                      <div key={item.label} className="flex items-center gap-3 px-4 py-3.5 hover:bg-ink-50 dark:hover:bg-ink-800/40 transition-colors">
                        {/* Item 27: 36px colored icon circle */}
                        ...
                        <div className="min-w-0">
                          ...
                          {item.href
                            /* Item 27: hover underline for links */
              ...
              {/* Item 28: proper pill badge tag cloud */}
              <AnimateIn delay={0.1}>
                <div className="bg-ink-50 dark:bg-ink-900/60 border border-ink-200 dark:border-ink-800 rounded-2xl p-5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {serviceTags.map(s => (
                      <span key={s.label} className={`text-xs font-[500] border px-[14px] py-[6px] rounded-full transition-all cursor-default hover:-translate-y-0.5 ${s.color}`}>
                        {s.label}
                      </span>
                    ))}
                  </div>
            ...
            <AnimateIn delay={0.12}>
              {status === 'sent' ? (
                ...
                <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl overflow-hidden shadow-card">
                  ...
                  {/* Item 25: consistent 48px inputs, 8px radius, labels above each field */}
                  <form onSubmit={handleSubmit} noValidate className="px-7 py-6 flex flex-col gap-5">
                    ...
                    {/* Item 26: Service dropdown — same styling as other inputs + custom chevron */}
                    <div>
                      ...
                      <div className="relative">
                        ...
                        {/* Item 26: custom chevron icon replacing default browser arrow */}
                        <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-ink-400 dark:text-ink-500">
                          <ChevronDown size={16} />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Item 29: full-width on mobile, loading spinner, success state already above */}
                    <button
                      type="submit"
                      disabled={status === 'sending'}
                      className="group w-full inline-flex items-center justify-center gap-2 text-sm font-[700] text-white bg-brand-500 hover:bg-brand-600 disabled:opacity-60 px-6 rounded-xl transition-all duration-200 hover:shadow-brand hover:-translate-y-px active:scale-[0.97] shine"
                      style={{ height: '48px' }}
                    >
                      {status === 'sending'
