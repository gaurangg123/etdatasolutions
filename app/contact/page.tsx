  ...
  if (status==='sent') return (
    <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl shadow-card overflow-hidden">
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.5,ease:[0.22,1,0.36,1]}}
        ...
        <div>
          <h3 className="text-2xl font-[750] text-ink-900 dark:text-ink-100 mb-2">Message sent!</h3>
          <p className="text-ink-500 dark:text-ink-400 max-w-sm leading-relaxed">We&apos;ll reply within 24 hours with a scoped proposal and a clear timeline.</p>
        </div>
        ...
        <button onClick={()=>{setStatus('idle');setForm(EMPTY)}} className="text-sm font-[500] text-ink-500 underline underline-offset-2 hover:text-brand-500 transition-colors">Send another message</button>
      </motion.div>
    </div>
  )
    ...
    <div className="bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl overflow-hidden shadow-card">
      ...
      <form onSubmit={handleSubmit} noValidate className="px-5 py-5 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[{id:'name',label:'Full Name',type:'text',ph:'Your full name',ac:'name',req:true},{id:'email',label:'Email',type:'email',ph:'you@company.com',ac:'email',req:true}].map(f=>(
            <div key={f.id}>
              <label htmlFor={f.id} className="block text-[0.72rem] font-[700] tracking-[0.07em] uppercase text-ink-500 dark:text-ink-400 mb-1.5">{f.label} {f.req&&<span className="text-brand-500">*</span>}</label>
              <input id={f.id} name={f.id} type={f.type} required={f.req} placeholder={f.ph} autoComplete={f.ac} value={(form as Record<string,string>)[f.id]} onChange={handleChange} className={cls(f.id)}/>
              {fe(f.id)&&<p className="mt-1 text-xs text-red-500">{fe(f.id)}</p>}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[{id:'company',label:'Company',type:'text',ph:'Company name',ac:'organization'},{id:'phone',label:'Phone',type:'tel',ph:'+1 000 000 0000',ac:'tel'}].map(f=>(
            <div key={f.id}>
        ...
        <div>
          ...
          <textarea id="message" name="message" required rows={4} placeholder="Describe your project, volume, timeline, or any questions…" value={form.message} onChange={handleChange}
            className={cn('w-full bg-[#fafaf9] dark:bg-ink-800 border rounded-lg px-4 py-3 text-sm text-ink-900 dark:text-ink-100 placeholder:text-ink-400 dark:placeholder:text-ink-600 outline-none transition-all duration-200 resize-y min-h-[100px] leading-relaxed',
        ...
        {status==='error'&&<div className="flex items-start gap-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4"><AlertTriangle size={15} className="text-red-500 flex-shrink-0 mt-0.5"/><p className="text-sm text-red-700 dark:text-red-400">{serverError||'Something went wrong.'}</p></div>}
        <button type="submit" disabled={status==='sending'}
          className={`group w-full inline-flex items-center justify-center gap-2 text-sm font-[700] text-white bg-brand-500 hover:bg-brand-600 px-6 rounded-xl transition-all hover:shadow-brand hover:-translate-y-px active:scale-[0.97] shine h-12 ${status==='sending'?'opacity-80 cursor-not-allowed':''}`}>
          {status==='sending'
...
export default function ContactPage() {
  return (
    <>
        ...
        <Container className="relative py-10 md:py-14">
          <AnimateIn>
            <nav className="flex items-center gap-1.5 mb-4 text-xs text-ink-400">
              ...
              <span>/</span><span className="text-ink-600 dark:text-ink-400 font-[500]">Contact</span>
            </nav>
            ...
            {/* BUG 9A: "clear timeline" */}
            <p className="text-lg text-ink-500 dark:text-ink-400 leading-relaxed max-w-[500px]">
              Tell us your requirement — we&apos;ll respond within 24 hours with a scoped proposal and a clear timeline.
            </p>
          </AnimateIn>
        </Container>
      </section>
            ...
            <div className="flex flex-col gap-4">
                    ...
                    {contactDetails.map(item=>(
                      <div key={item.label} className="flex items-center gap-3 px-4 py-3 hover:bg-ink-50 dark:hover:bg-ink-800/40 transition-colors">
                        ...
                        <div className="min-w-0">
                          ...
                          {item.href ? <a href={item.href} className="text-sm font-[500] text-ink-700 dark:text-ink-300 hover:text-brand-500 transition-colors truncate block underline-offset-2 hover:underline">{item.val}</a>
              ...
              <AnimateIn delay={0.1}>
                <div className="flex items-start gap-3 bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 rounded-2xl p-4">
                  ...
                  <div><p className="text-sm font-[700] text-ink-900 dark:text-ink-100 mb-0.5">We respond in 24 hours</p><p className="text-xs text-ink-500 dark:text-ink-400">All enquiries receive a personalized response.</p></div>
                </div>
              </AnimateIn>
            </div>
            {/* Right — form */}
      ...
      {/* BUG 9B: Free audit card */}
      <section id="audit" className="snap-section bg-ink-50 dark:bg-[#0a0908]">
        <Container className="py-10">
          <AnimateIn>
            <div className="max-w-2xl mx-auto bg-white dark:bg-ink-900 border border-brand-200 dark:border-brand-500/25 rounded-3xl p-8 text-center shadow-card">
              <div className="w-12 h-12 rounded-2xl bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 flex items-center justify-center text-brand-500 mx-auto mb-4">
                <Timer size={22}/>
              </div>
              <p className="text-xs font-[700] uppercase tracking-[0.08em] text-brand-500 mb-2">Free Operations Audit</p>
              ...
              <p className="text-sm text-ink-500 dark:text-ink-400 leading-relaxed mb-5 max-w-md mx-auto">Book a free 30-minute operations audit — we&apos;ll map your workflow, identify your 3 biggest bottlenecks, and give you a cost-savings estimate. No commitment.</p>
