...
const cases = [
  { metric:'$40K saved annually', secondary:'Invoice processing: 14 days → 3 days', tags:['Data Entry','Logistics'], color:'#0ea5e9',
  ...
  { metric:'Zero P1 bugs on launch', secondary:'Dev team shipped 3 weeks ahead of schedule', tags:['QA Testing','SaaS'], color:'#10b981',
  ...
  { metric:'$2.3M unbilled found', secondary:'Weekly reporting: 18 hours → 25 minutes', tags:['Data Engineering','Healthcare'], color:'#8b5cf6',
    ...
    metrics:[{l:'Found',v:'$2.3M'},{l:'Reporting',v:'25 min'},{l:'Sources',v:'5'}] },
]
const processSteps = [
...
export default function AboutPage() {
  return (
    <>
        ...
        <Container className="relative py-10 md:py-14">
          <AnimateIn>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/services" className="inline-flex items-center gap-2 text-sm font-[650] text-ink-700 dark:text-ink-300 bg-white dark:bg-ink-800 border border-ink-200 dark:border-ink-700 hover:border-brand-400 hover:text-brand-500 px-5 py-2.5 rounded-xl transition-all">View services</Link>
            </div>
          </AnimateIn>
        </Container>
      </section>
      ...
      <section className="snap-section bg-ink-900 dark:bg-ink-950">
        <Container className="py-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 rounded-2xl overflow-hidden">
            {[{n:'10+',label:'Years Active',sub:'Operating since 2014'},{n:'99%',label:'Accuracy Rate',sub:'Multi-level QC'},{n:'100+',label:'Clients Served',sub:'US, UK, CA, AU'},{n:'24/7',label:'Availability',sub:'Any timezone'}].map((s,i)=>(
      ...
      {/* BUG 10: Values with lucide icons */}
              ...
              <AnimateIn key={i} delay={i*0.1}>
                <div className="flex flex-col h-full bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-2xl overflow-hidden hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <div className="text-[1.5rem] font-[850] tracking-[-0.05em] leading-none mb-1" style={{color:cs.color}}>{cs.metric}</div>
                    <p className="text-sm font-[600] text-ink-600 dark:text-ink-400 mb-3">{cs.secondary}</p>
                    <div className="flex flex-wrap gap-1.5 mb-4">{cs.tags.map(t=><span key={t} className="text-[0.68rem] font-[700] tracking-[0.08em] uppercase text-ink-400 bg-ink-100 dark:bg-ink-800 px-2 py-0.5 rounded-full">{t}</span>)}</div>
                    <div className="space-y-3 flex-1">
                      <div className="bg-ink-50 dark:bg-ink-800/60 rounded-xl p-3"><p className="text-[0.65rem] font-[700] uppercase tracking-[0.09em] text-ink-400 mb-1">The problem</p><p className="text-xs text-ink-600 dark:text-ink-400 leading-relaxed">{cs.problem}</p></div>
                      <div className="bg-ink-50 dark:bg-ink-800/60 rounded-xl p-3"><p className="text-[0.65rem] font-[700] uppercase tracking-[0.09em] text-ink-400 mb-1">What we did</p><p className="text-xs text-ink-600 dark:text-ink-400 leading-relaxed">{cs.did}</p></div>
                    </div>
      ...
      {/* BUG 6C: Process + FIX 16 connectors */}
      <section id="process" className="snap-section-tall bg-white dark:bg-ink-950">
        <Container className="py-10 md:py-14">
            ...
            {processSteps.map((s,i)=>(
              <div key={s.n} className="relative">
                ...
                {i < processSteps.length-1 && (
                  <>
                    <div aria-hidden className="hidden lg:block absolute top-[22px] h-px border-t-2 border-dashed border-ink-200 dark:border-ink-700 z-20" style={{left:'calc(50% + 28px)',right:'calc(-50% + 28px)'}} />
          ...
          <AnimateIn delay={0.35} className="mt-8 text-center">
            <Link href="/contact" className="inline-flex items-center gap-2 text-sm font-[700] text-white bg-brand-500 hover:bg-brand-600 px-6 py-3 rounded-xl transition-all hover:-translate-y-px hover:shadow-brand shine">Start your project today</Link>
          </AnimateIn>
        </Container>
      </section>
      <D />
            ...
            <div className="flex flex-col gap-3">
              {chipGroups.map((group,gi)=>(
                <div key={gi} className="flex flex-wrap gap-2">
                  {group.chips.map(c=><span key={c} className={`text-sm font-[550] border px-4 py-1.5 rounded-full cursor-default hover:-translate-y-0.5 hover:shadow-sm transition-all ${group.color}`}>{c}</span>)}
                </div>
              ))}
            </div>
          </AnimateIn>
