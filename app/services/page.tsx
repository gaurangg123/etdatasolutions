...
// Item 22: normalized tag pill colors per vertical
const accents: Record<string, {
  iconBg: string; iconText: string; iconBorder: string;
  tagBg: string; tagText: string; tagBorder: string;
  accentColor: string; numColor: string;
}> = {
  staffing: {
    iconBg:'bg-violet-50 dark:bg-violet-500/10', iconText:'text-violet-600 dark:text-violet-400', iconBorder:'border-violet-200 dark:border-violet-500/25',
    ...
    accentColor: '#8b5cf6', numColor: 'text-violet-100/5 dark:text-violet-100/5',
  },
  data: {
    iconBg:'bg-sky-50 dark:bg-sky-500/10', iconText:'text-sky-600 dark:text-sky-400', iconBorder:'border-sky-200 dark:border-sky-500/25',
    ...
    accentColor: '#0ea5e9', numColor: 'text-sky-100/5 dark:text-sky-100/5',
  },
  qa: {
    iconBg:'bg-emerald-50 dark:bg-emerald-500/10', iconText:'text-emerald-600 dark:text-emerald-400', iconBorder:'border-emerald-200 dark:border-emerald-500/25',
    ...
    accentColor: '#10b981', numColor: 'text-emerald-100/5 dark:text-emerald-100/5',
  },
  dataeng: {
    iconBg:'bg-amber-50 dark:bg-amber-500/10', iconText:'text-amber-600 dark:text-amber-400', iconBorder:'border-amber-200 dark:border-amber-500/25',
    ...
    accentColor: '#f59e0b', numColor: 'text-amber-100/5 dark:text-amber-100/5',
  },
}
    ...
    <>
      ...
      <section className="relative overflow-hidden py-20 md:py-28 bg-white dark:bg-ink-950">
        ...
        <Container className="relative">
          ...
          {/* Item 5: H1 at proper large scale */}
      ...
      {services.map((svc, idx) => {
        ...
        // Item 21: alternating left/right layout
        const isReversed = idx % 2 === 1
        return (
          <div key={svc.id}>
            <Section bg={isReversed ? 'gray' : 'white'} id={svc.id} size="lg" label={svc.title}>
              <Container>
                {/* Item 21: alternating layout — odd sections flip content/card sides */}
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-14 items-start ${isReversed ? 'lg:grid-flow-dense' : ''}`}>

                  {/* Text column — item 21: alternate order */}
                  <div className={isReversed ? 'lg:col-start-2' : ''}>
                    ...
                    {/* Item 23: checkmark icons in colored circles */}
                    <AnimateIn delay={0.2}>
                    ...
                    {svc.highlight && (
                      <AnimateIn delay={0.23}>
                        <div className={`flex items-start gap-3 ${a.iconBg} border ${a.iconBorder} border-l-2 rounded-xl p-3.5 mb-6`}
                          style={{ borderLeftColor: a.accentColor }}>
                    ...
                    {/* Item 22: normalized tag pill padding: 6px 14px, 12px, weight 500 */}
                    <AnimateIn delay={0.25}>
                      <div className="flex flex-wrap gap-1.5 mb-7">
                        {svc.tags.map(t => (
                          <span key={t} className={`text-[0.75rem] font-[500] px-[14px] py-[6px] rounded-full border ${a.tagBg} ${a.tagText} ${a.tagBorder}`}>
                            {t}
                          </span>
                        ))}
                      </div>
                  ...
                  {/* Delivery card — item 21: alternate to opposite side */}
                  <AnimateIn delay={0.15} className={isReversed ? 'lg:col-start-1 lg:row-start-1' : ''}>
                    <div className="relative bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl overflow-hidden shadow-card hover:shadow-card-hover transition-shadow duration-300">

                      {/* Item 24: large background watermark number (opacity 0.06) */}
                      <span aria-hidden className="absolute -bottom-4 -right-2 text-[9rem] font-[900] leading-none select-none pointer-events-none"
                        style={{ color: a.accentColor, opacity: 0.06 }}>
                        {svc.num}
                      </span>
                      ...
                      <div className="relative h-52 overflow-hidden">
                        ...
                        {/* Accent color stripe at top of image */}
                      ...
                      {/* Deliverables list — item 23: checkmark bullets */}
                      <ul className="divide-y divide-ink-100 dark:divide-ink-800 relative z-10">
                        {svc.items.map(item => (
                          <li key={item} className="flex items-center gap-3 px-5 py-3 hover:bg-ink-50 dark:hover:bg-ink-800/40 hover:pl-6 transition-all duration-200">
                            <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${a.iconBg} ${a.iconText}`}>
