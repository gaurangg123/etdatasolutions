...
import { ArrowRight, Users, Database, CheckSquare, BarChart2, Check, Clock, Lock, Globe, ChevronRight, Quote, Star } from 'lucide-react'
...
const testimonials = [
  {
    quote: "ET Data Solutions has been an absolute game-changer for our data processing workflow. The accuracy is consistently above 99% and turnaround times are exceptional.",
    name: "Sarah Mitchell",
  ...
  {
    quote: "We&apos;ve been using their QA testing services for 2 years. They catch issues our automated tools miss. The team is professional, communicative and incredibly thorough.",
    name: "James Kowalski",
    role: "VP Engineering",
  ...
  {
    quote: "Their staffing team placed 3 VAs with us within a week. The quality of candidates and onboarding support made the whole process seamless. Highly recommended.",
    name: "Priya Sharma",
    role: "CEO",
...
export default function HomePage() {
  ...
  return (
    <>
          ...
          <motion.div {...fade(0.28)} className="grid grid-cols-2 md:grid-cols-4 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden max-w-[560px] mx-auto gap-px bg-neutral-200 dark:bg-neutral-800">
            {stats.map(s => (
              <div key={s.l} className="bg-white dark:bg-[#0e0c0b] px-5 py-4 hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-colors duration-150 text-left">
                <div className="text-[1.6rem] font-[800] tracking-[-0.05em] text-brand-500 leading-none mb-1">
                  <CountUp value={s.n} />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden bg-neutral-200 dark:bg-neutral-800 gap-px">
            {services.map((svc, i) => (
              <AnimateIn key={svc.id} delay={i * 0.07}>
                <Link href={`/services#${svc.id}`} className="group relative flex flex-col h-full bg-white dark:bg-[#111] hover:bg-neutral-50 dark:hover:bg-[#151515] transition-colors duration-200 overflow-hidden">
                  {/* Service image */}
                  <div className="relative h-44 overflow-hidden">
                    ...
                    <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-brand-500 to-brand-600 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500" />
                    {/* Icon overlay on image */}
                    <div className="absolute bottom-3 left-4 w-10 h-10 rounded-xl bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white">
                      {svcIcons[svc.id]}
                    </div>
                    <span className="absolute bottom-3 right-4 text-[0.61rem] font-[700] tracking-[0.14em] uppercase text-white/60">{svc.num}</span>
                  </div>

                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-[1rem] font-[700] tracking-[-0.02em] text-neutral-900 dark:text-neutral-100 mb-2">{svc.title}</h3>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400 leading-relaxed mb-4 flex-1">{svc.description.slice(0, 110)}…</p>
        ...
        <div className="max-w-[1180px] mx-auto px-6">
          <AnimateIn>
            <div className="relative rounded-3xl overflow-hidden h-[340px] md:h-[420px]">
              ...
              <div className="absolute inset-0 flex flex-col justify-center px-10 md:px-16 max-w-[520px]">
                <span className="text-[0.67rem] font-[700] tracking-[0.12em] uppercase text-brand-400 mb-4 block">Our team</span>
                ...
                <p className="text-white/70 text-sm leading-relaxed mb-6 max-w-[380px]">
                  Our India-based team of operators, supervisors, and specialists work around the clock to deliver precision results for clients worldwide.
                </p>
                <Link href="/about" className="inline-flex items-center gap-2 text-sm font-[600] text-white border border-white/30 hover:bg-white/15 px-5 py-2.5 rounded-xl transition-all duration-150 w-fit">
                  Meet the company <ArrowRight size={14} />
                </Link>
              </div>
            </div>
          </AnimateIn>
      ...
      <section className="py-24 bg-white dark:bg-[#0e0c0b]">
        <div className="max-w-[1180px] mx-auto px-6">
          <AnimateIn><span className="text-[0.67rem] font-[700] tracking-[0.12em] uppercase text-brand-500 mb-3 block">Client testimonials</span></AnimateIn>
          <AnimateIn delay={0.07}><h2 className="text-3xl md:text-4xl font-[800] tracking-[-0.04em] text-neutral-900 dark:text-neutral-50 leading-[1.1] mb-12">Trusted by businesses<br />around the world</h2></AnimateIn>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {testimonials.map((t, i) => (
              <AnimateIn key={i} delay={i * 0.1}>
                <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 hover:border-neutral-300 dark:hover:border-neutral-700 hover:shadow-[0_8px_24px_rgba(0,0,0,0.07)] dark:hover:shadow-[0_8px_24px_rgba(0,0,0,0.3)] transition-all duration-250">
                  {/* Stars */}
                  ...
                  {/* Quote icon */}
                  <Quote size={20} className="text-brand-500/30 mb-3 flex-shrink-0" />
                  {/* Text */}
                  <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed flex-1 mb-5" dangerouslySetInnerHTML={{ __html: t.quote }} />
                  {/* Attribution */}
                  <div className="flex items-center gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-800">
                    <div className="w-9 h-9 rounded-full bg-brand-100 dark:bg-brand-500/20 flex items-center justify-center text-brand-600 dark:text-brand-400 text-sm font-[700] flex-shrink-0">
                      {t.name.charAt(0)}
                    </div>
                    <div>
                      <div className="text-sm font-[700] text-neutral-900 dark:text-neutral-100 tracking-[-0.01em]">{t.name}</div>
                      <div className="text-xs text-neutral-400 dark:text-neutral-500">{t.role}, {t.company}</div>
                    </div>
                  </div>
                </div>
              </AnimateIn>
