...
export default function HomePage() {
  return (
    <>
      {/* Fix 14: Each section wrapped in snap-section */}
      <Hero />
      <TrustBar />
      ...
      <section className="snap-section bg-white dark:bg-ink-950">
        <Container className="py-8 md:py-12">
          <AnimateIn className="text-center mb-6">
            ...
            <h2 className="text-[1.8rem] sm:text-[2.2rem] font-[800] tracking-[-0.04em] text-ink-900 dark:text-ink-50 leading-tight">Four services. One reliable partner.</h2>
          </AnimateIn>
          {/* Fix 13: responsive grid */}
              ...
              <AnimateIn key={s.label} delay={i * 0.08}>
                ...
                <Link href={s.href} className="group flex flex-col gap-3 p-5 bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-2xl hover:border-brand-300 dark:hover:border-brand-500/40 hover:-translate-y-1.5 hover:shadow-lg transition-all duration-300 h-full">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-[750] text-ink-900 dark:text-ink-100 mb-1 leading-snug">{s.label}</p>
                    <p className="text-xs text-ink-500 dark:text-ink-400 leading-relaxed">{s.desc}</p>
                  </div>
                  <span className="inline-flex items-center gap-1.5 text-xs font-[650] text-brand-500 group-hover:gap-2.5 transition-all">
      ...
      <section className="snap-section bg-ink-50 dark:bg-[#0a0908]">
        <Container className="py-8 md:py-12">
          <AnimateIn className="text-center mb-6">
            ...
            <h2 className="text-[1.8rem] sm:text-[2.2rem] font-[800] tracking-[-0.04em] text-ink-900 dark:text-ink-50 leading-tight">Real clients. Numbers that don&apos;t lie.</h2>
          </AnimateIn>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {results.map((r, i) => (
              <AnimateIn key={r.val} delay={i * 0.1}>
                <div className="group bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-2xl p-5 hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                  <div className="flex items-center gap-3 mb-3">
                    <r.icon size={18} style={{ color: r.color }} />
                    <span className="text-[1.8rem] font-[850] tracking-[-0.05em] leading-none" style={{ color: r.color }}>{r.val}</span>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {previews.map((p, i) => (
              <AnimateIn key={p.name} delay={i * 0.07}>
                <Link href="/testimonials" className="group flex flex-col gap-3 p-4 bg-ink-50 dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-2xl hover:border-brand-300 dark:hover:border-brand-500/40 hover:-translate-y-0.5 hover:shadow-card transition-all duration-200 h-full">
      ...
      <section className="snap-section bg-ink-50 dark:bg-[#0a0908]">
        <Container className="py-8 md:py-12">
          ...
          {/* Fix 9: connecting dashed line */}
          <div className="grid grid-cols-5 gap-2 relative">
            {steps.map((s, i) => (
              <AnimateIn key={s.n} delay={i * 0.07}>
                <div className="relative flex flex-col items-center text-center gap-2">
                  <div className="w-9 h-9 rounded-xl bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 flex items-center justify-center text-brand-500 flex-shrink-0 z-10">
                    <s.icon size={15} />
                  </div>
                  {i < steps.length - 1 && (
      ...
      {/* CTA Banner — Fix 4: only ONE banner */}
      <section className="snap-section bg-white dark:bg-ink-950">
        <CtaBanner
