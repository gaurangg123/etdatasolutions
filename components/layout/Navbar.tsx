      ...
      >
        <div className="max-w-container mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-2.5 flex-shrink-0 group min-w-0" onClick={() => setOpen(false)}>
            ...
            {/* Logo text with proper vertical centering — item 2 */}
            <div className="hidden sm:flex items-center gap-2.5 min-w-0">
              <div className="min-w-0">
                <span className="block text-[1rem] font-[750] tracking-[-0.025em] text-ink-900 dark:text-ink-100 group-hover:text-brand-500 transition-colors leading-tight truncate">
                  ET Data Solutions
                </span>
                <span className="block text-[0.68rem] text-ink-400 dark:text-ink-500 font-[500] tracking-[0.02em] leading-tight">
                  Outsourcing · Est. 2014
                </span>
              </div>
              {/* Subtle divider between logo and nav links — item 2 */}
              <div className="hidden md:block w-px h-7 bg-ink-200 dark:bg-ink-700 mx-1 flex-shrink-0" />
            </div>
          </Link>

          {/* Desktop nav — item 32: active link marking */}
            ...
            {links.map(l => {
              ...
              return (
                <Link key={l.href} href={l.href}
                  className={cn(
                    'nav-link-underline relative text-sm font-[500] px-3.5 py-2 rounded-lg transition-all duration-150 whitespace-nowrap',
                    isActive
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-4 h-[2.5px] rounded-full bg-brand-500" />
                  )}
                </Link>
              )
            })}
          ...
          <div className="flex items-center gap-2">
            {/* Phone — desktop only — item 1: always shown */}
            <a
              href="tel:+13023579776"
              className="hidden lg:flex items-center gap-1.5 text-xs font-[600] text-ink-500 dark:text-ink-400 hover:text-brand-500 transition-colors mr-1 group"
            >
              <Phone size={13} className="transition-transform duration-200 group-hover:scale-110" />
            ...
            {/* CTA — desktop — item 3: single primary CTA */}
