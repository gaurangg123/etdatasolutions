          ...
          {cases.map((cs, i) => (
            <AnimateIn key={i} delay={i * 0.1}>
              <div className="group flex flex-col h-full bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-3xl overflow-hidden hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300">
                ...
                {/* Top — colored result bar */}
                <div className={`${cs.color} px-5 py-4`}>
                  <div className="flex items-start gap-2.5">
                    <cs.icon size={16} className="text-white/80 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-[700] text-white leading-snug">{cs.result}</p>
                      <p className="text-xs text-white/70 mt-0.5">{cs.sub}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex gap-2 mt-5 flex-wrap">
                    {cs.metrics.map(m => (
                      <div key={m.label} className="flex-1 min-w-[70px] text-center bg-ink-50 dark:bg-ink-800 border border-ink-200 dark:border-ink-700 rounded-xl py-2.5 px-2">
                        <div className="text-base font-[800] tracking-[-0.04em] text-ink-900 dark:text-ink-100 leading-none mb-0.5">{m.val}</div>
                        <div className="text-[0.65rem] text-ink-400 font-[600] tracking-[0.05em] uppercase">{m.label}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-ink-100 dark:border-ink-800">
                    <Button href="/contact#form" variant="ghost" size="sm" arrow className="px-0 text-brand-500">
                      Get similar results
                    </Button>
                  </div>
                </div>
