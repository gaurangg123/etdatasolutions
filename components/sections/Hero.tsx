      ...
      <div className="relative" style={{ minHeight: 'clamp(120px, 20vw, 200px)' }}>
        <AnimatePresence mode="wait">
          <motion.h1
            ...
            className="absolute inset-x-0 text-[clamp(1.8rem,5vw,4rem)] font-[800] tracking-tight leading-[1.1] text-ink-900 dark:text-white text-balance"
          >
            {HEADLINES[idx]}
          </motion.h1>
        </AnimatePresence>
...
export default function Hero() {
  ...
  return (
    <section id="hero" ref={ref} className="snap-section relative overflow-hidden bg-white dark:bg-ink-950">
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none opacity-[0.3] dark:opacity-0" style={{ backgroundImage: 'radial-gradient(circle, rgba(0,0,0,0.06) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 w-full">
        <Container className="py-8 md:py-12">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px] gap-8 xl:gap-14 items-center">
            ...
            {/* Dashboard card — hidden on mobile */}
          ...
          <motion.div {...f(0.32)} className="mt-6 lg:hidden grid grid-cols-2 gap-px bg-ink-200 dark:bg-white/8 border border-ink-200 dark:border-white/10 rounded-2xl overflow-hidden">
            {stats.map(s => (
              <div key={s.label} className="bg-white dark:bg-ink-900 px-4 py-3">
                <div className="text-[1.5rem] font-[850] tracking-[-0.05em] text-brand-500 leading-none mb-0.5"><CountUp value={s.n} /></div>
